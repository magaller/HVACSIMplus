     ...
     $     rwork1, rwork2, dnorm, itrmjv,IFLAG,IBLK,ISBLK)

      implicit none  

      integer n, ipar(*), ijacv, ifdord, itask, nfe, njve, nrpre, 
     $     itrmjv
      real xcur(n), fcur(n), rpar(*), v(n), z(n), 
...
c ------------------------------------------------------------------------
     ...
     $     nfe, rwork1, z, rwork2, dnorm, itrmjv,IFLAG,IBLK,ISBLK)
      endif
      njve = njve + 1
c ------------------------------------------------------------------------
...
c ------------------------------------------------------------------------
 900  continue
     ...
     $     nfe, v, z, rwork, dnorm, itrmjv,IFLAG,IBLK,ISBLK)
...
c ------------------------------------------------------------------------
      if (ijacv .eq. 0 .or. ifdord .eq. 1) then 
         eps = sqrt((1.e0 + dnorm(n,xcur,1))*epsmach)/eps
         do 100 i = 1, n
            v(i) = xcur(i) + eps*v(i)
 100     continue
	   IFLAG=1
         call f(n, v, z, IFLAG,IBLK,ISBLK)
	   itrmf=0	!ZC
         if (itrmf .ne. 0) then
            itrmjv = 1
            goto 900
...
c ------------------------------------------------------------------------
      if (ifdord .eq. 2) then 
         eps = (((1.e0 + dnorm(n,xcur,1))*epsmach)**(1.e0/3.e0))/eps
         do 200 i = 1, n
            rwork(i) = xcur(i) + eps*v(i)
 200     continue
	   IFLAG=1
         call f(n, rwork, z, IFLAG,IBLK,ISBLK) 
	   itrmf=0	!ZC
         nfe = nfe + 1
         if (itrmf .ne. 0) then
            itrmjv = 1
 ...
 210     continue
	   IFLAG=1
         call f(n, rwork, v, IFLAG,IBLK,ISBLK) 
	   itrmjv=0	!ZC
         nfe = nfe + 1
         if (itrmf .ne. 0) then
            itrmjv = 1
...
c ------------------------------------------------------------------------
      if (ifdord .eq. 4) then 
         eps = (((1.e0 + dnorm(n,xcur,1))*epsmach)**(1.e0/5.e0))/eps
         do 300 i = 1, n
            rwork(i) = xcur(i) + eps*v(i)
 300     continue
	   IFLAG=1
	   ...
	   IFLAG=1
         call f(n, xcur, rwork, IFLAG,IBLK,ISBLK) 
	   itrmf=0	!ZC
         nfe = nfe + 1
         if (itrmf .ne. 0) then
            itrmjv = 1
