     ...
     $	 IBLK,ISBLK)
      ...
      real x(n), ftol, stptol, rwork(*), rpar(*) 
      real dinpr, dnorm
...
!      if (input(10) .ge. 0 .and. input(10) .le. 3) then 
      if (input(10) .ge. 0 .and. input(10) .le. 5) then	!ZC+ 
         ieta = input(10)
      else
         iterm = -10
         go to 900
...
c ------------------------------------------------------------------------
      if ( choice1_exp .le. 1.0e0 .or. choice1_exp .gt. 2.0e0 ) 
     &  choice1_exp = DFLT_CHOICE1_EXP
      if ( choice2_exp .le. 1.0e0 .or. choice2_exp .gt. 2.0e0 ) 
     &  choice2_exp = DFLT_CHOICE2_EXP
      if ( choice2_coef .lt. 0.0e0 .or. choice2_coef .gt. 1.0e0 ) 
     &  choice2_coef = DFLT_CHOICE2_COEF
      if ( etamax .le. 0.0e0 ) etamax = DFLT_ETA_MAX
      if ( thmin .lt. 0.0e0 .or. thmin .gt. thmax )
     &  thmin = DFLT_THMIN
      if ( thmax .gt. 1.0e0 .or. thmax .lt. thmin )
     &  thmax = DFLT_THMAX
c ------------------------------------------------------------------------
...
c ------------------------------------------------------------------------
     ...
     $        rwork(lrwork), dinpr, dnorm,IFLAG,IBLK,ISBLK)
c ------------------------------------------------------------------------
