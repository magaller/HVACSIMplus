     ...
     $     itrmbt,IFLAG,IBLK,ISBLK)

      implicit none 

      integer n, nfe, ibt, ibtmax, ipar(*), itrmbt
      real xcur(n), fcnrm, step(n), eta, 
...
c ------------------------------------------------------------------------
 ...
 110  continue
	IFLAG=1
      call f(n, xpls, fpls, IFLAG,IBLK,ISBLK) 
	itrmf=0	!ZC
      if (itrmf .ne. 0) then
         itrmbt = 2
         go to 900
...
c ------------------------------------------------------------------------
      if (fpnrm .le. (1.e0 - t*(1.e0-eta))*fcnrm .or. ibtmax .eq. -1) 
     $ then 
         itrmbt = 0
         go to 900 
      endif
...
c ... choose theta ...
c ------------------------------------------------------------------------
      theta = -(oftjs*redfac)/(fpnrm**2 - fcnrm**2 - 2.e0*oftjs*redfac)
