     ...
     $     njve, nrpre, nli, kdmax, kdmaxp1, itrmks
      real xcur(n), fcur(n), fcnrm, step(n), eta, rpar(*), 
...
c ------------------------------------------------------------------------
c 
      real abstol, big, cs, cndmax, epsmach, rsnrm0, 
...
c ------------------------------------------------------------------------
      if (ncall .eq. 0) then
         epsmach = 2.0e0*dlamch( 'e' )
         ncall = 1
         cndmax = 1.e0/(100.e0*epsmach)
      endif
      do 20 i = 1, n
         step(i) = 0.e0
 20   continue
...
c ------------------------------------------------------------------------
     ...
     $     rwork, rwork, dnorm, itrmjv,IFLAG,IBLK,ISBLK)
      if (itrmjv .gt. 0) then 
...
c ------------------------------------------------------------------------
      ...
      temp = max(min(w(kd)/rsnrm,1.0e0),-1.0e0) 
...
c ------------------------------------------------------------------------
     ...
     $        vv(1,kdp1), vv(1,kdp1), dnorm, itrmjv,iflag,iblk,isblk)
         if (itrmjv .gt. 0) then 
            itrmks = 2
            go to 900
         endif
...
c ------------------------------------------------------------------------
     ...
     $        rwork, vv(1,kdp1), dnorm, itrmjv,IFLAG,IBLK,ISBLK)
...
c ------------------------------------------------------------------------
      ...
      temp = float(kd)*log(abstol/(rsnrm0*rsnrm))/
     $     log(rsnrm/(1.e0 + 10.e0*epsmach))
      if (temp .ge. 1000.e0*float(iksmax - igm)) then 
         itrmks = 4
         go to 900
      endif
c ------------------------------------------------------------------------
...
c ------------------------------------------------------------------------
 900  continue
c ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
c	save termination condition (ZC)
	itrmks_rec(iblk)=itrmks
c ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
