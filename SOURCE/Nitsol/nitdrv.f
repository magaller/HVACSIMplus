     ...
     $     rwork, dinpr, dnorm, iflag,iblk,isblk)
	...
	use nist_solver	!(ZC)
     ...
     $     nrpre, nli, nni, nbt
      real xcur(n), fcur(n), xpls(n), fpls(n), 
...
c 
      real alpha, epsmach, eta, etamin, fcnrm, 
...
c For printing:
	real ttt
	common /nitsoltime/ ttt
      integer infe, injve, inrpre, inli
c ------------------------------------------------------------------------
      data ncall / 0 /
      save ncall, alpha, gamma, epsmach 
c	Interface variable (ZC)
	integer iflag,iblk,isblk
c	Ill-conditioned recorder (ZC)
	integer iterm_rec(10),itrmks_rec(10)
	common /info_saved/ iterm_rec,itrmks_rec	
c ------------------------------------------------------------------------
c Initialize.
c ------------------------------------------------------------------------
      if ( ncall .eq. 0 ) epsmach = 2.0e0*dlamch( 'e' )
      ncall = 1
...
c ------------------------------------------------------------------------
	IFLAG=1
      call f(n, xcur, fcur, IFLAG,IBLK,ISBLK)
	itrmf=0	!ZC
      if ( itrmf .ne. 0 ) then
         iterm = 2
         go to 900
...
c For printing:
      if (iplvl .ge. 1) then 
	   ...
	   write(ipunit,*) 'time=',ttt,'  iblk=',iblk	!ZC write time
      endif
c ------------------------------------------------------------------------
c
c ------------------------------------------------------------------------
...
c         if ( nni .eq. 0 ) then
c            write(ipunit,799) nni, njve, 0.0e0, fcnrm
c         else 
...
c	Extra step tolerance if stptol is too small (ZC)
      if (nni.gt.0 .and. itrmks.eq.0) then 
	if (stpnrm .le. epsmach) then
	   iterm = 7
	   go to 900
	endif
	endif
...
!	if (nfe .ge. nnimax) then	!nnimax set to max nfe to compare with PH
         iterm = 1
         go to 900
      endif
c ------------------------------------------------------------------------
...
c ------------------------------------------------------------------------
     ...
     $        rsnrm, dinpr, dnorm, itrmks,IFLAG,IBLK,ISBLK)
      endif
c ------------------------------------------------------------------------
...
c ------------------------------------------------------------------------
      krystat = itrmks
      avrate = (rsnrm/fcnrm)**(1.0e0/real(nli))
c ------------------------------------------------------------------------
...
c ------------------------------------------------------------------------
      ...
      if (itrmks .ge. 3) then 
         if (rsnrm/fcnrm .ge. 1.e0) then 
            iterm = 5
            go to 900
         else
            temp = log(ftol/fcnrm)/
     $           log(rsnrm/((1.e0 + 10.e0*epsmach)*fcnrm))
            if (temp .gt. 1000.e0*float(nnimax - nni)) then 
               iterm = 5
               go to 900
            endif
         endif
...
c ------------------------------------------------------------------------
      if (ieta .eq. 0) then 
         etamin = eta**alpha
         temp = 1.e0 - redfac
         flmnrm = sqrt((temp*fcnrm)**2 + 2.e0*redfac*temp*oftlm + 
     $        (redfac*rsnrm)**2)
         eta = abs(fpnrm - flmnrm)/fcnrm 
      endif
      if (ieta .eq. 1) then 
         etamin = eta**2
         eta = (fpnrm/fcnrm)**2
...
c ------------------------------------------------------------------------
 900  continue
	iterm_rec(iblk)=iterm	!Record iterm (ZC)
c ------------------------------------------------------------------------
c For printing:
      if (iplvl .ge. 1) then 
         write(ipunit,*) 
